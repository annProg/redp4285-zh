...
```
```{#fig:process_thread_thread .plot:dot caption="线程创建" width=48% height=180px subfig=1}
digraph task_struct {
    rankdir="TB"
    compound=true
...
TASK_INTERRUPTIBLE
:    在这种状态下，进程被挂起并等待某个条件得到满足。如果进程处于 TASK_INTERRUPTIBLE 状态，当进程收到停止信号时，进程状态改变，操作中断。TASK_INTERRUPTIBLE 进程的一个典型的例子是等待键盘中断的进程

TASK_UNINTERRUPTIBLE
:    类似于 TASK_INTERRUPTIBLE。不同于处于 TASK_INTERRUPTIBLE 状态的进程可以被中断，发送信号对 TASK_UNINTERRUPTIBLE 状态的进程没有任何作用。TASK_UNINTERRUPTIBLE 进程的一个典型示例是等待磁盘 I/O 操作的进程。
